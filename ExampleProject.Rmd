...
```{r setup-conditional-output-limit, include=FALSE}
library(knitr)
library(evaluate)

knit_hooks$set(output = function(x, options) {
  if (isTRUE(options$limit_output)) {
    lines <- strsplit(x, "\n")[[1]]
    truncated <- head(lines, 5)
    if (length(lines) > 5) truncated <- c(truncated, "... [output truncated]")
